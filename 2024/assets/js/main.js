...
(function () {
  ...
  $(window).on('load', function () {
    // $('body').addClass('-beforeLoadedFade');
  ...
  clipboard.on('success', function () {
    setTimeout(() => {
      $('[data-copy]').addClass('-yl').text('複製成功');
    }, 500);
  });
  ...
  if (result.indexOf('Chrome') === -1 && result.indexOf('Safari') === -1) {
    ...
    $('[data-popup="browser"]').addClass('-show');
    return;
  }
  ...
  // 登入
  $('[data-login]').on('click.login', function (e) {
    if (!$('body').hasClass('-isLogin')) {
      e.preventDefault();
      $('body').addClass('-loginOpen');
    }
  });
  $('.login').on('click.loginClose', function (e) {
